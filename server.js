...
// ============================================
// CONFIGURACIÓN DE PROXY (CORREGIDO)
// ============================================
...
class SecurityDefender {
    ...
    validatePhone(phone) {
        ...
        console.log('📱 Solo dígitos:', digitsOnly, '→ Longitud:', digitsOnly.length);
        
        // Verificar que tenga entre 7 y 16 dígitos
        if (digitsOnly.length < 7 || digitsOnly.length > 16) {
            console.log('❌ Teléfono: longitud inválida (' + digitsOnly.length + ' dígitos)');
            return false;
        }
        
        // Patrones fraudulentos obvios
        const fraudPatterns = [
            /^(\d)\1{9,}$/,
            /^1234567890$/,
            /^0987654321$/,
        ...
        if (fraudPatterns.some(p => p.test(digitsOnly))) {
            console.log('❌ Teléfono: patrón fraudulento');
            return false;
        }
        
        console.log('✅ Teléfono ACEPTADO:', digitsOnly);
        return true;
    }
...
if (!SUPABASE_URL || !SUPABASE_KEY) {
    ...
    console.error('SUPABASE_URL:', SUPABASE_URL ? 'Configurada' : 'NO CONFIGURADA');
    console.error('SUPABASE_SERVICE_ROLE_KEY:', SUPABASE_KEY ? 'Configurada' : 'NO CONFIGURADA');
    process.exit(1);
}
...
app.post('/api/verify-admin', (req, res) => {
    ...
    console.log('🔑 Intento de login. Contraseña recibida:', password ? 'Sí' : 'No');
...
app.post('/api/catalogos', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'Error al crear catálogo: ' + error.message });
    }
});
...
app.post('/api/contactos', async (req, res) => {
    ...
    // Validar teléfono con logs
    const phoneValid = security.validatePhone(telefono);
    console.log('📱 Validación teléfono:', telefono, '→', phoneValid ? 'VÁLIDO' : 'INVÁLIDO');
    
    if (!phoneValid) {
        return res.status(400).json({ error: 'Número de teléfono inválido. Use formato: +53 5357 1569' });
    }
...
app.get('/api/acercade', async (req, res) => {
    try {
        ...
        console.log('📄 Acerca de cargado:', data ? 'Sí' : 'No');
        res.json(data || {});
    } catch (error) {
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`✅ Validación telefónica: ACTIVA (modo flexible)`);
