...
const CACHE_DURATIONS = {
    catalogos: 5 * 60 * 1000,    // 5 minutos
    acercade: 10 * 60 * 1000,    // 10 minutos
...
function getCached(key) {
    ...
    if (Date.now() - entry.timestamp > (CACHE_DURATIONS[key] || 300000)) {
        cache.delete(key);
        return null;
    }
    return entry.data;
...
function setCache(key, data) {
    // Limpiar caché si hay demasiadas entradas (máximo 50)
    if (cache.size > 50) {
...
class SecurityDefender {
    ...
    detectBruteForce(ip, endpoint) {
        ...
        if (recent.length >= 10) {
            ...
            // Auto-desbloquear después de 1 hora
            setTimeout(() => {
...
app.use(helmet({
    contentSecurityPolicy: {
        directives: {
            ...
            connectSrc: ["'self'", "https://pacho-fg.onrender.com"],
            frameSrc: ["'none'"],
            objectSrc: ["'none'"],
...
const apiLimiter = rateLimit({
    windowMs: 60 * 1000,
    max: 30,
    message: { error: 'Demasiadas solicitudes a la API' },
    validate: { trustProxy: false }
});
...
// ============================================
// API DE CATÁLOGOS (CON CACHÉ)
// ============================================

app.get('/api/catalogos', async (req, res) => {
    const cacheKey = 'catalogos';
...
// ============================================
// API DE PEDIDOS (CON CACHÉ PARA GET)
// ============================================
...
app.post('/api/pedidos', async (req, res) => {
    ...
    let { cliente, telefono, email, producto, detalles, estado, fecha } = req.body;
    
    console.log('📦 Pedido recibido:', { cliente, telefono, producto });
    
    cliente = security.sanitizeInput(cliente);
    telefono = security.sanitizeInput(telefono);
    email = security.sanitizeInput(email || 'No especificado');
...
// ============================================
// API DE ACERCA DE (CON CACHÉ)
// ============================================

app.get('/api/acercade', async (req, res) => {
    const cacheKey = 'acercade';
    const cachedData = getCached(cacheKey);
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`✅ Caché en memoria: ACTIVADA`);
    console.log(`✅ Caché de recursos: CONFIGURADA`);
