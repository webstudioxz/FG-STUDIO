...
class SecurityDefender {
    ...
    validatePhone(phone) {
        ...
        console.log('📱 Validando teléfono:', phone, '→ Limpio:', clean);
        
        // Verificar longitud mínima y máxima
        if (clean.length < 7 || clean.length > 16) {
            console.log('❌ Teléfono: longitud inválida (' + clean.length + ' dígitos)');
            return false;
        }
        ...
        if (fraudPatterns.some(p => p.test(clean))) {
            console.log('❌ Teléfono: patrón fraudulento detectado');
            return false;
        }
        
        // Aceptar todos los formatos válidos
        console.log('✅ Teléfono aceptado:', clean);
        return true;
    }
